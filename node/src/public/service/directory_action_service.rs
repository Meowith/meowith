...
pub async fn do_rename_directory(
    ...
    bucket_accessor.has_permission(&e_path.bucket_id, &e_path.app_id, *RENAME_DIRECTORY_ALLOWANCE)?;
    let path = e_path.path();

    if path == req.path() { // The paths equal, no work needs to be done
        return Ok(());
    }

    if path.is_empty() || req.path().is_empty() { // no touching the root "dir"
    ...
    if get_directory(e_path.bucket_id, Some(req.path().clone()), &app_state.session).await.is_ok()
    {
    ...
    let new_parent = try_mkdir(e_path.bucket_id, new_parent.unwrap_or(String::new()), &app_state.session).await?;

    let child_stream = DirectoryIterator::from_parent(original_directory.clone(), &app_state.session);
    ...
    let new_parent_path = join_parent_name(&new_parent.clone().map(|d| d.full_path()).unwrap_or(String::new()), &new_name);

    for dir in to_rename {
        let new_local_parent_path = normalize((new_parent_path.clone() + "/" + &old_parent_path.clone()[old_parent_path.len()..]).as_str());
        let _new_dir = update_directory_path(&dir, Some(new_local_parent_path), dir.name.clone(), &app_state.session).await?;
    }

    let _new_dir = update_directory_path(&original_directory, new_parent.map(|d| d.full_path()), new_name, &app_state.session).await?;
