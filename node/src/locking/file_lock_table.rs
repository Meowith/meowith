...
pub struct FileLockTable<K: Sized + Eq + std::hash::Hash + Clone, T: Sized> {
    lock_table: Arc<LockTable<K, T>>,
    max_readers: u32,
}

#[allow(unused)]
impl<K: Sized + Eq + std::hash::Hash + Clone, T: Sized + KyeAbleValue<K>> FileLockTable<K, T> {
    fn new() -> Self {
        FileLockTable {
    ...
    async fn try_read(&self, key: K) -> TryLockResult<FileReadGuard<'_, K, T>> {
        match self.lock_table.lock().await.entry(key.clone()) {
            Entry::Occupied(lock_entry) => {
                lock_entry.get().try_read()
            }
            Entry::Vacant(entry) => {
                let data = T::new(&key);
                let lock = FileLock::new(Arc::downgrade(&self.lock_table), self.max_readers, key, data);
    ...
    async fn try_write(&self, key: K) -> TryLockResult<FileWriteGuard<'_, K, T>> {
         match self.lock_table.lock().await.entry(key.clone()) {
            Entry::Occupied(lock_entry) => {
                lock_entry.get().try_write()
            }
            Entry::Vacant(entry) => {
                let data = T::new(&key);
                let lock = FileLock::new(Arc::downgrade(&self.lock_table), self.max_readers, key, data);
...
pub struct FileLock<K: Sized + Eq + std::hash::Hash, T: Sized> {
    locker: Arc<Locker<K, T>>,
    data: UnsafeCell<T>,
}

impl<K: Sized + Eq + std::hash::Hash, T: Sized> FileLock<K, T> {
...
pub(crate) struct Locker<K: Sized + Eq + std::hash::Hash, T: Sized> {
...
#[allow(unused)]
impl<K: Sized + Eq + std::hash::Hash, T: Sized> Locker<K, T> {
    pub(crate) fn release_read(&self) {
        if let Some(lock_table) = self.lock_table.upgrade() {
            ...
            if self.semaphore.available_permits() + 1 == self.max_readers as usize {
                ...
                let mut table = lock_table.blocking_lock();
                let _ = table.remove(&self.key);
            }
        }
    }
