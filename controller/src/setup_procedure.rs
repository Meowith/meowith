...
use crate::setup::auth_routes::{EmptyResponse, login, register};
...
pub async fn setup_controller(clonfig: ControllerConfig, auth: AuthMethodMap, ssl: Option<SslAcceptorBuilder>, session: CachingSession) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(ssl) = ssl {
        setup_server_future = setup_server
            .bind_openssl(
                (clonfig.setup_addr.clone(), clonfig.setup_port),
                ssl,
            )?
            .run();
        info!(
